...
class Route {
   ...
   */
  scan(fn, init) {
    return this.via(Flow.scan(fn, init));
  }
...
export class Source extends Route {
  ...
  reduce(fn, init) {
    return this.runWith(Sink.reduce(fn, init));
  }
...
export class Sink {
  ...
  static reduce(fn, init) {
    return new Sink(new ReduceSink(fn, init));
  }
...
export class Flow extends Route {
  ...
  static scan(fn, init) {
    return new Flow(new ScanFlow(fn, init));
  }
